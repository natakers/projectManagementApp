      ...
      { 
        (!error) 
          ? (boards.map((board: BoardProps) => (
            <Board toggleWindow={toggleWindow} id={board.id} key={board.id} title={board.title} description={board.description} />
          ))) 
          : (<div className="text-red-300">Oops! Something does wrong!</div>)
      }
      {isOpen && <ModalWindow boardId={currentId} toggleWindow={toggleWindow}  />}
    </div>
  );
};
